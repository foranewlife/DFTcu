...
3.  **交换相关势 (V_xc)**: 对于包含噪声或负波动的电荷密度（如初始原子叠加），QE 会使用电荷密度的绝对值 `abs(rho)` 计算 $V_{xc}$ 以保证数值稳定性。
4.  **局部势 (V_loc)**: 必须严格在 $G$ 空间应用 `ecutrho` 截断，否则在原子中心会产生巨大的吉布斯纹波。
5.  **标准流程**: 使用 `dftcu.initialize_hamiltonian` 函数可自动处理上述对齐细节，实现 $10^{-14}$ Ha 级别的精度。
