...
__global__ void lda_pz_kernel(int n, const double* rho, double* v_xc, double* energy_density) {
    ...
    if (i < n) {
        ...
        if (rs < 1.0) {
            ...
            vc = log_rs * (a + (2.0/3.0) * c * rs) + b - (1.0/3.0) * a + (1.0/3.0) * (2.0 * d - c) * rs;
        } else {
            ...
            ec = gamma / denom;
            vc = (gamma + (7.0/6.0 * gamma * beta1) * rs_sqrt + (4.0/3.0 * gamma * beta2) * rs) / (denom * denom);
        }
...
__global__ void reduce_sum_kernel(const int n, const double* data, double* partial_sums, const int block_size) {
...
double LDA_PZ::compute(const RealField& rho, RealField& v_xc) {
    ...
    reduce_sum_kernel<<<grid_size, block_size, block_size * sizeof(double)>>>(n, energy_density.data(), partial_sums.data(), block_size);
    GPU_CHECK_KERNEL

    std::vector<double> h_partial_sums(grid_size);
    partial_sums.copy_to_host(h_partial_sums.data());
    double total_energy = 0.0;
    for (double s : h_partial_sums) total_energy += s;
