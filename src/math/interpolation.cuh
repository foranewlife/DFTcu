...
inline CubicSplineCoefficients precompute_cubic_spline_coefficients(
    ...
    for (int i = 0; i < coeff.n - 1; ++i) {
        ...
        if (coeff.h[i] <= 0.0) {
            throw std::invalid_argument("precompute_cubic_spline_coefficients: x_data must be strictly increasing");
        }
    }
 ...
 */
__host__ __device__ inline double cubic_spline_interpolate_device(
    double x, const double* x_data, const double* y_data,
    const double* M, const double* h, int n) {
...
inline double cubic_spline_interpolate(double x, const std::vector<double>& x_data,
    ...
    for (int i = 0; i < n - 1; ++i) {
        ...
        if (h[i] <= 0.0) {
            throw std::invalid_argument("cubic_spline_interpolate: x_data must be strictly increasing");
        }
    }

    // 构建三对角方程组求解二阶导数 M[i]
    // 方程：h[i-1]*M[i-1] + 2*(h[i-1]+h[i])*M[i] + h[i]*M[i+1] = 6*((y[i+1]-y[i])/h[i] - (y[i]-y[i-1])/h[i-1])
    // 自然边界条件：M[0] = M[n-1] = 0
    ...
    for (int i = 1; i < n - 1; ++i) {
        ...
        b[i] = 2.0 * (h[i - 1] + h[i]);
        c[i] = h[i];
        d[i] = 6.0 * ((y_data[i + 1] - y_data[i]) / h[i] -
...
inline double linear_interpolate_uniform(double x, double x0, double dx,
    ...
    // 边界处理
    if (x <= x0) return y_data[0];
    if (x >= x0 + (n - 1) * dx) return y_data[n - 1];
