...
def real_spherical_harmonics(l, m, gx, gy, gz, g):
    if l == 0: return np.ones_like(g) * np.sqrt(1.0 / (4.0 * np.pi))
    if l == 1:
        c = np.sqrt(3.0 / (4.0 * np.pi))
        mask = g > 1e-12
        res = np.zeros_like(g)
        if m == 0: res[mask] = c * gz[mask] / g[mask]
        elif m == 1: res[mask] = c * gx[mask] / g[mask]
        elif m == -1: res[mask] = c * gy[mask] / g[mask]
        return res
    return np.zeros_like(g)

def hankel_transform(q, r, rab, f_r, l):
    res = np.zeros_like(q)
    for i, qi in enumerate(q):
        if qi < 1e-8:
            if l == 0: res[i] = np.sum(f_r * r * rab)
            else: res[i] = 0.0
        else:
            res[i] = np.sum(f_r * spherical_jn(l, qi * r) * r * rab)
    return res
...
def test_nonlocal_alignment():
    ...
    for i, b in enumerate(betas_upf):
        ...
        f_r = b["content"]
        f_q = hankel_transform(g, r_grid, rab, f_r, l)
        prefix = (4.0 * np.pi / grid.volume()) * ((-1j)**l)
        m_list = [0] if l == 0 else [0, 1, -1]
        d_val = dij_ry[i, i] * 0.5
        for m in m_list:
            ylm = real_spherical_harmonics(l, m, gx, gy, gz, g)
            proj = prefix * ylm * f_q
